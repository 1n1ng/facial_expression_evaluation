...
def calculate_au6(landmarks):
    point_263 = np.array(landmarks[263])
    point_374 = np.array(landmarks[374])
...
def main():
    ...
    if au_values:
        ...
        reference_data = {
            'au6': float(mean_values[0]),
            'au12': float(mean_values[1]),
            'au16': float(mean_values[2]),
            'au25': float(mean_values[3]),
            'au26': float(mean_values[4])
        }
